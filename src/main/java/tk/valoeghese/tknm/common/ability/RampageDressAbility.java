...
public class RampageDressAbility extends AbstractElectromasterAbility {
	@Override
	...
	@Override
	public int[] performAbility(World world, PlayerEntity player, int level, float abilityProgress, byte usage, AbstractElectromasterAbility.Data data) {
		if (level > 0) {
			if (player.getStackInHand(Hand.MAIN_HAND).isEmpty()) {
				boolean charged = AbstractElectromasterAbility.CHARGED.getBoolean(player.getUuid());

				// add extra abilities here: strength, speed, etc

				return AbstractElectromasterAbility.performAlterCharge(world.getTime(), player, charged ? CHARGE_OFF : CHARGE_ON);
			}
		}
