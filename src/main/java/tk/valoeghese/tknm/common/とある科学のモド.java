...
public class とある科学のモド implements ModInitializer {
	...
	public void onInitialize() {
		...
		ServerSidePacketRegistry.INSTANCE.register(USE_ABILITY_PACKET_ID, (context, dataManager) -> {
			...
			ability.performAbility(player.world, player, component.getLevel(), usage);

			// main thread task queue
			context.getTaskQueue().execute(() -> {
				PacketByteBuf data = new PacketByteBuf(Unpooled.buffer());
				data.writeDoubleLE(player.getX());
				data.writeDoubleLE(player.getY());
				data.writeDoubleLE(player.getZ());
				data.writeFloatLE(player.yaw);
				data.writeFloatLE(player.pitch);
				data.writeIdentifier(abilityId);
				data.writeByte(usage);

				PlayerStream.around(player.world, player.getPos(), 420.0).forEach(pe -> {
					ServerSidePacketRegistry.INSTANCE.sendToPlayer(pe, RENDER_ABILITY_PACKET_ID, data);
				});
			});
		});
	}
